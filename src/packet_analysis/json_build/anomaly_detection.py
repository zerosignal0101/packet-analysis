...
def get_anomalies(file, environment,hostIP):
    ...
    for index, row in df.iterrows():
        ...
        df_row['hostip'] = hostIP  # 主机IP
        df_row['class_method'] = row['request_type']
        df_row['anomaly_delay'] = "{:.6f}".format(row['Time_since_request'])
        df_row['average_delay'] = "{:.6f}".format(average)
...
def process_anomalies(file_paths, environment,hostIP):
    all_details = []

    for file_path in file_paths:
        if file_path and 'anomalies' in file_path:
            print(f"Processing file: {file_path}")
            details = get_anomalies(file_path, environment,hostIP)

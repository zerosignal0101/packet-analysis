...
from src.packet_analysis.utils.cache import with_pcap_lock, get_file_hash, get_redis_client
    ...
    if use_cache:
        try:
            ...
            if redis_client.exists(cache_key):
                logger.info(f"Cache hit for {pcap_file} (hash: {file_hash}). Returning cached result.")
                ...
                logger.info(f"Cache miss for {pcap_file} (hash: {file_hash}).")
